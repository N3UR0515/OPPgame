    ...
    public Map(int cols, int rows)
    {
        ...
        for (int i = 0; i < initTileCount; i++)
        {
            ...
            if (tempCol % 2 == 0) {
                ...
                tiles[tempCol * this.rows + tempRow] = factory.createTile(0, true, tempRow, tempCol, ""); // cia sukurt random viena kart key ir pora kart heal
            } else {
                if (tiles[tempCol * rows + tempRow] != null) {
                    i--;
                    continue;
                ...
                coordSave.add(tempCol + "_" + tempRow);
                tiles[tempCol * this.rows + tempRow] = factory.createTile(0, false, tempRow, tempCol, ""); // cia sukurt random viena kart key ir pora kart heal
            }
        }
        ...
        for (Tile tile : roomTiles){
            if (isUnavailable(tile)) {
                tiles[tile.getTrel_x() * this.rows + tile.getTrel_y()] = factory.createTile(0, tile.id, tile.getTrel_y(), tile.getTrel_x(), ""); // cia sukurt random viena kart key ir pora kart heal
            }
        }
        ...
        while (prec[currentTile.getTrel_x() * this.rows + currentTile.getTrel_y()] != currentTile) {
                ...
                for (Tile n : neighbors){
                    if (isUnavailable(n)) {
                        if (isFiery && rng.nextInt(16) == 4) {
                            ...
                            tiles[n.getTrel_x() * this.rows + n.getTrel_y()] = factory.createTile(0, n.id, n.getTrel_y(), n.getTrel_x(), ""); // cia sukurt random viena kart key ir pora kart heal
                        }
                    }
                }
            }
            if (isUnavailable(currentTile)) {
                if (isFiery && rng.nextInt(16) == 4){
                    ...
                    tiles[currentTile.getTrel_x() * this.rows + currentTile.getTrel_y()] = factory.createTile(0, currentTile.id, currentTile.getTrel_y(), currentTile.getTrel_x(), ""); // cia sukurt random viena kart key ir pora kart heal
                }
            }
